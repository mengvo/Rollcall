...
struct MatchView: View {
    ...
    private func sendToMatchingService(image: UIImage) {
        // Endpoint expects { "img_base64": "..." }
        guard let imageData = image.jpegData(compressionQuality: 0.9) else { return }
        let base64String = imageData.base64EncodedString()
        
        guard let body = try? JSONEncoder().encode(["img_base64": base64String]) else { return }
        
        isProcessing = true
        
        var request = URLRequest(url: URL(string: "http://10.23.28.242:8000/match/")!)
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            DispatchQueue.main.async {
                ...
                // Handle 404 (no match found) gracefully
                if let http = response as? HTTPURLResponse, http.statusCode == 404 { return }
                
                guard let result = try? JSONDecoder().decode(MatchResult.self, from: data) else { return }
                
                // Deduplicate — only add if this person isn't already matched
                if !matchedPeople.contains(where: { $0.id == result.id }) {
                    matchedPeople.append(result)
                    
                    // Show banner for new match
                    latestMatch = result
                    DispatchQueue.main.asyncAfter(deadline: .now() + 4) {
                        if latestMatch?.id == result.id {
                            latestMatch = nil
                        }
                    }
                }
